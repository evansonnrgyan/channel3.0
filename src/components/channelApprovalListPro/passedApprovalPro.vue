<template>
  <div class="channelApprovalPro">
    ...
    <div class="channelApprovalPro_pages" v-if="pendingData.length>0">
      <Page  ref='pagess2' :total="total" :current="current"  @on-change="changePage" @on-page-size-change="changeSize" show-total show-elevator show-sizer class="page" ></Page>
    </div>
  </div>
</template>
<style lang="less">
  ...
  export default{
    ...
    methods:{
      ...
      search(){
        if(!!this.alllist){
          this.$nextTick(()=>{
             this.$refs['pagess2']?this.$refs['pagess2'].current=1:""
          })
