<template>
  <div class="contentner demo">
    <div class="portal_plate_down new_proClass info-bg">
      ...
      <div class="info-shadow info-box selinput channel-demo-header-search-body">
        <Row class="bot14">
          <Col span="8">
        ...
        <Row class="bot14">
          <Col span="8">
            ...
            <Input v-model="searchObj.product.productNum" placeholder="请输入"></Input>
          </Col>
          <!-- <Col span="8">
          <label for="">业务类型</label>
          <Select v-model="searchObj.businessType" clearable>
            <Option v-for="item in businessTypeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
          </Col> -->
          <Col span="8">
            <label for="">省市</label>
            <Select v-model="searchObj.primateProvinceCode" clearable class="selectSpc" @on-change='getSubCity(searchObj.primateProvinceCode)'>
              <Option v-for="item in provinceList" :value="item.value+''" :key="item.value">{{ item.label }}</Option>
            </Select>
            <Select v-model="searchObj.secondProvinceCode" clearable class="selectSpc">
        ...
        <Row class="bot14">
          <Col span="10">
            ...
            <Select v-model="searchObj.primateMainBrandCode" filterable clearable class="selectSpc" @on-change='getBrand(searchObj.primateMainBrandCode)'>
              <Option v-for="item in mainBrandList" :value="item.value" :key="item.value">{{ item.label }}</Option>
            </Select>
            <Select v-model="searchObj.secondMainBrandCode" clearable filterable class="selectSpc">
      ...
      <div class="info-shadow info-box">
        <div class="optionArea">
          <Button type="primary" @click="openNewTab">New Tab</Button>
          <vbl-auth-wrap :options="auth" :url="url" http-type="post" class="permissionStyle">
            <vbl-auth name="test2"><Button type="primary">test2</Button></vbl-auth>
          </vbl-auth-wrap>
          <vbl-auth-wrap :options="auth" :url="url" http-type="post" class="permissionStyle">
            <vbl-auth name="test"><Button type="primary">test</Button></vbl-auth>
          </vbl-auth-wrap>
    ...
    <Modal v-model="addModalVisible" :mask-closable="false" :title="modalTitle">
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80" class='addform'>
        ...
        <!-- <FormItem label="业务类型" prop="businessType">
          <Select v-model="formValidate.businessType" clearable>
            <Option v-for="item in businessTypeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
        </FormItem> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      openNewTab () {
        this.$bus.$emit("newTab", {"id": "newTab", isShow: true, "name":"tab页中打开新标签页", params:{"id":"111"}});
      },
